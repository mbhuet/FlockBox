...
namespace CloudFine
{
    public class LeaderFollowBehavior : GlobalBehavior
    {
        ...
        public override void GetSteeringBehaviorVector(out Vector3 steer, SteeringAgent mine, SurroundingsContainer surroundings)
        {
            ...
            float scalar = Vector3.Dot(mine.Position - closestLeader.Position, closestLeader.Forward.normalized);
            if (scalar > 0 && scalar < clearAheadDistance)//we are somewhere in front of the leader, potentially in the clear zone ahead of it.
            {
                pointOnLeaderPath_cached = closestLeader.Position + closestLeader.Forward.normalized * scalar;
