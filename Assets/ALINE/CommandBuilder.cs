...
namespace Drawing {
	...
	public partial struct CommandBuilder : IDisposable {
		...
		/// <param name="normal">Direction perpendicular to the plane.</param>
		...
		public void SolidPlane (float3 center, float3 normal, float2 size) {
			SolidPlane(center, Quaternion.LookRotation(calculateTangent(normal)), size);
		}
		...
		public void WirePlane (float3 center, float3 normal, float2 size) {
			WirePlane(center, Quaternion.LookRotation(calculateTangent(normal)), size);
		}
		...
		public void PlaneWithNormal (float3 center, float3 normal, float2 size) {
			PlaneWithNormal(center, Quaternion.LookRotation(calculateTangent(normal)), size);
		}
		...
		internal struct Builder : IJob {
			...
			unsafe void AddText (System.UInt16* text, TextData textData, Color32 color) {
				...
				for (int i = 0; i < textData.numCharacters; i++) {
					...
					maxBounds = math.max(minBounds, v);
